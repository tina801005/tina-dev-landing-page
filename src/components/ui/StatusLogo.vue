...
<template>
    <div class="flex h-16 items-center justify-between px-6">
      ...
      <a href="#top" class="group relative flex items-center gap-2 cursor-pointer">
        ...
        <div class="absolute w-50 top-full left-0 mt-2 scale-95 opacity-0 pointer-events-none transition-all duration-200 ease-out 
        group-hover:scale-100 group-hover:opacity-100 group-hover:translate-y-1 rounded-lg bg-slate-900 border border-slate-800 px-3 py-1.5 text-xs text-slate-200 shadow-xl">
          ...
          <span class="flex items-center gap-1.5">
            <span :class="['h-1 w-1 rounded-full', isOnline ? 'bg-emerald-500' : 'bg-slate-500']"></span>
            {{ statusText }}
          </span>
        </div>
